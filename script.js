...
function startGame(selectedDifficulty) {
    ...
    }, 10000); // Показує результати через 10 секунд після початку гри
}
...
for (let i = 0; i < answer_buttons.length; i++) {
    answer_buttons[i].addEventListener("click", function () {
        ...
        if (selected_answer === current_question.correct) {
            correct_answer += 1;
            answer_buttons[i].style.background = "#6be809"; // Зелений колір для правильного
        } else {
            answer_buttons[i].style.background = "#ef0009"; // Червоний для неправильного
        }
        total_answer += 1;

        // Генеруємо нове питання через 500 мс після вибору відповіді
        setTimeout(() => {
            current_question = new Question(difficulty);
            current_question.display();
        }, 500);
